...
fn extract_template_stem_strips_extensions() {
    use std::path::Path;
    assert_eq!(
        md_tmpl::extract_template_stem(Path::new("review.tmpl.md")),
        "review"
    );
    assert_eq!(
        md_tmpl::extract_template_stem(Path::new("path/to/check.tmpl.md")),
        "check"
    );
    assert_eq!(
        md_tmpl::extract_template_stem(Path::new("simple.md")),
        "simple"
    );
    assert_eq!(md_tmpl::extract_template_stem(Path::new("bare")), "bare");
}
...
body"#;
    let (fm, _) =
        parse_frontmatter(source).expect("quoted types and alternative brackets should succeed");
...
trait UnwrapErrOrElse<T> {
    fn unwrap_err_or_else<F: FnOnce()>(self, f: F) -> md_tmpl::TemplateError;
}

impl<T: std::fmt::Debug> UnwrapErrOrElse<T> for Result<T, md_tmpl::TemplateError> {
    fn unwrap_err_or_else<F: FnOnce()>(self, f: F) -> md_tmpl::TemplateError {
        if let Err(e) = self {
            e
        } else {
