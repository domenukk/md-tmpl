//! Criterion benchmarks for the `md-tmpl` crate.
//!
//! Groups:
...
fn small_context() -> md_tmpl::Context {
    ctx! {
        name: "Alice",
        place: "Wonderland",
    }
...
fn medium_context() -> md_tmpl::Context {
    ctx! {
        title: "Monthly",
        status: "complete",
        score: 87.456_f64,
...
fn large_context() -> md_tmpl::Context {
    use std::sync::Arc;
...
fn bench_flexbuffers(c: &mut Criterion) {
    ...
    group.bench_function("deser_small", |b| {
        b.iter(|| md_tmpl::Context::from_flexbuffers(black_box(&small_fb)).unwrap());
    });

    group.bench_function("deser_medium", |b| {
        b.iter(|| md_tmpl::Context::from_flexbuffers(black_box(&medium_fb)).unwrap());
    });

    // Measure full FlexBuffers → render path vs pre-built Context → render.
    group.bench_function("render_small_from_flexbuffers", |b| {
        b.iter(|| {
            let ctx = md_tmpl::Context::from_flexbuffers(black_box(&small_fb)).unwrap();
    ...
    group.bench_function("render_medium_from_flexbuffers", |b| {
        b.iter(|| {
            let ctx = md_tmpl::Context::from_flexbuffers(black_box(&medium_fb)).unwrap();
