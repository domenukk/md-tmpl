...
prompt-templates converter:

1. ``@variant`` decorator — turn a class with annotations into a matchable
   variant with ``__match_args__`` and the ``_prompt_template_tag`` protocol.
...
Example::
    ...
    from prompt_templates import variant, Variants, load_types
...
def variant(cls: type) -> type:
    ...
    - ``_prompt_template_tag`` (class attribute, defaults to class name)
    - ``_prompt_template_fields`` property (dict of field values)
    ...
    name = cls.__name__
    tag = getattr(cls, "_prompt_template_tag", name)
    ...
    namespace["_prompt_template_tag"] = tag
    ...
    def make_fields_prop(fields: list[str]) -> Any:
        ...
        def _prompt_template_fields(self: Any) -> dict[str, Any]:
            return {f: getattr(self, f) for f in fields}

        return _prompt_template_fields

    namespace["_prompt_template_fields"] = make_fields_prop(field_names)
...
class _UnitSentinel:
    ...
    Compared by tag name. Carries ``_prompt_template_tag`` and empty
    ``_prompt_template_fields`` for converter compatibility.
    """

    __slots__ = ("_prompt_template_tag", "_prompt_template_fields")

    def __init__(self, tag: str) -> None:
        self._prompt_template_tag = tag
        self._prompt_template_fields: dict[str, Any] = {}
...
class Variants(metaclass=_VariantsMeta):
    ...
    All variants carry ``_prompt_template_tag`` and
    ``_prompt_template_fields`` for converter compatibility.
    """
...
def load_types(
    ...
    Example::
        ...
        from prompt_templates import load_types
