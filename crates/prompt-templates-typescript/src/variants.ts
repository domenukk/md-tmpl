 ...
 * @example
 * ```ts
 * import { unitVariant, variant, defineVariants } from "prompt-templates";
 *
 * // --- variant() ---
 * const NeedsChanges = variant("NeedsChanges", ["reason"]);
 * const v = NeedsChanges({ reason: "fix tests" });
 * console.log(v._prompt_template_tag); // "NeedsChanges"
...
export interface VariantInstance {
  readonly _prompt_template_tag: string;
  readonly _prompt_template_fields: Readonly<Record<string, unknown>>;
  [key: string]: unknown;
}
 ...
 * `_prompt_template_tag` / `_prompt_template_fields` protocol.
 *
 * @example
 * ```ts
 * const Approved = unitVariant("Approved");
 * console.log(Approved._prompt_template_tag); // "Approved"
 * ```
 */
export function unitVariant(tag: string): VariantInstance {
  return Object.freeze({
    _prompt_template_tag: tag,
    _prompt_template_fields: Object.freeze({}),
    toString() {
      return tag;
    },
  });
...
export function variant<F extends string>(
  ...
  const ctor = (fields: Record<F, unknown>): VariantInstance => {
    ...
    const instance: VariantInstance = {
      _prompt_template_tag: tag,
      _prompt_template_fields: Object.freeze({ ...fieldsObj }),
      ...fieldsObj,
      toString() {
...
export function match<R>(
  ...
  if (typeof value === "string") {
    ...
    if (typeof obj._prompt_template_tag === "string") {
      ...
      tag = obj._prompt_template_tag;
      fields = (obj._prompt_template_fields as Record<string, unknown>) ?? {};
 ...
 *
 * Works with both the `_prompt_template_tag` protocol and `__kind__`
 * tagged objects.
 *
 * @example
 * ```ts
...
export function isVariant(value: unknown, variantName: string): boolean {
  ...
  if (value !== null && typeof value === "object") {
    const obj = value as Record<string, unknown>;
    if (obj._prompt_template_tag === variantName) return true;
